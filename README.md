...
**3단계 (구단 운영) — 순서**: 연봉/계약 ✔ → 드래프트 ✔ → **FA**(다음) → 트레이드, +2군/재정.
- 연봉/계약([DESIGN_CONTRACTS.md](DESIGN_CONTRACTS.md)): WAR×$/WAR 가치평가·동적 예산·경쟁균형세.
- 드래프트([DESIGN_DRAFT.md](DESIGN_DRAFT.md)): 역순(Z자) 지명·Need/BPA·스카우팅 불확실성.
  30시즌 순위 순환(영구지배/바닥 없음) 확인. `python scripts/draft_check.py`
- 다음: **FA**(자격 서비스타임 훅·입찰가=`value_of` 활용) → 트레이드(지명권↔선수 가치비교).
이후 4단계: 웹 UI.
