...
import FACompensationBoard from './FACompensationBoard.jsx'
...
export default function Offseason({ state, onState }) {
  ...
  const load = async () => {
    setLoading(true)
    const [reports, tradeState, faState, compensationState, draftState] = await Promise.all([
      api.offseason().catch(() => []),
      api.tradeState().catch(() => null),
      api.faState().catch(() => null),
      api.compensationState().catch(() => null),
      api.draftState().catch(() => null),
    ])
    setReport(reports)
    setTrade(tradeState?.active ? tradeState : null)
    setFa(faState?.active ? faState : null)
    setCompensation(compensationState?.active ? compensationState : null)
  ...
  const completeFA = async (result) => {
    setFa(null)
    setReport(await api.offseason().catch(() => []))
    if (result.compensation_active) {
      const next = await api.compensationState().catch(() => null)
      setCompensation(next?.active ? next : null)
    } else if (result.draft_active) {
      const draftState = await api.draftState().catch(() => null)
      setDraft(draftState?.active ? draftState : null)
    } else {
  ...
  const completeCompensation = async (result) => {
  ...
  return <div className="offseason-page">
    ...
    {!trade && !fa && compensation && <FACompensationBoard market={compensation}
      onMarketChange={setCompensation} onComplete={completeCompensation} />}
    {!trade && !fa && !compensation && draft && <DraftBoard draft={draft}
      onDraftChange={setDraft} onComplete={completeDraft} />}
    ...
    <section className="card">
      ...
      {report.length === 0
        ? <p className="muted">첫 시즌이 끝나면 에이징 → 트레이드 → FA → FA 보상 → 드래프트 → 재정 순서로 진행됩니다.</p>
        : report.map(stage => <div key={stage.stage}>
            <h3>{stage.stage}
              ...
              {stage.stage === 'FA 보상' && compensation && <span className="muted"> · 진행 중</span>}
