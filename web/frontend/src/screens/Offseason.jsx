...
export default function Offseason({ state, onState }) {
  ...
  const load = async () => {
    setLoading(true)
    const [reports, faState, draftState] = await Promise.all([
      api.offseason().catch(() => []),
  ...
  return <div className="offseason-page">
    {fa && <FABoard market={fa} onMarketChange={setFa} onComplete={completeFA} />}
    {!fa && draft && <DraftBoard draft={draft} onDraftChange={setDraft} onComplete={completeDraft} />}
